...
export default async function SpkDetailPage({ params }: { params: Params }) {
  ...
  const status = getOperationalStatus(spk, progressTotal, latestDate, daysRemaining);
  ...
  return (
    <main style={{ minHeight: '100vh', background: '#f6f8fc', color: '#0f172a' }}>
      ...
      <section style={{ padding: 28, maxWidth: 1380, margin: '0 auto' }}>
        ...
        <section style={gridFour}><Metric label="Progress Akumulasi" value={`${(progressTotal * 100).toFixed(1)}%`} /><Metric label="Progress Periode Terakhir" value={getLatestPeriod(historyRows)} /><Metric label="Target Selesai" value={spk.tgl_target_selesai} /><Metric label="Sisa Hari" value={daysRemaining < 0 ? `Lewat ${Math.abs(daysRemaining)} hari` : `${daysRemaining} hari`} /></section>
        <section style={{ ...card, marginTop: 16, padding: 18 }}><div style={{ display: 'flex', justifyContent: 'space-between', gap: 20, flexWrap: 'wrap', alignItems: 'center' }}><div><div style={mutedLabel}>STATUS OPERASIONAL</div><div style={{ marginTop: 8 }}><StatusBadge status={status} /></div></div><Info label="Tanggal SPK" value={spk.tgl_spk} /><Info label="Kantor/Pelaksana" value={office?.nama_kantor_pelaksana ?? spk.id_kantor} /><Info label="Mandor" value={mandor?.nama_mandor ?? spk.id_mandor} /><Info label="Status SPK" value={spk.status_spk} /></div></section>
...
function getOperationalStatus(spk: Spk, progress: number, latestDate: string | null, daysRemaining: number): OperationalStatus { if (spk.status_spk === 'SELESAI' || progress >= 0.999999) return 'SELESAI'; if (daysRemaining < 0) return 'LEWAT TARGET'; if (!latestDate || differenceInDays(latestDate, new Date().toISOString().slice(0, 10)) > 14 || daysRemaining <= 7) return 'PERHATIAN'; return 'BERJALAN'; }
...
function Info({ label, value }: { label: string; value: string }) { return <div><div style={mutedLabel}>{label}</div><div style={{ marginTop: 6, fontWeight: 700 }}>{value}</div></div>; }
