...
export default async function SpkDetailPage({ params, searchParams }: { params: Params; searchParams: SearchParams }) {
  ...
  const snapshotTotal = currentRows.reduce((sum, row) => sum + Number(row.bobot_final ?? 0), 0);
  const snapshotValid = currentRows.length > 0 && Math.abs(snapshotTotal - 1) <= 0.00001;
  ...
  return (
    <main className="spk-detail-page">
      <section className="spk-detail-content">
        ...
        <section className="spk-detail-snapshot-panel"><div className="spk-detail-section-head"><div><div style={sectionTitleNoPad}>Snapshot Bobot Final SPK</div><div style={mutedText}>Bobot ini adalah bobot final yang dipakai SPK untuk seluruh perhitungan progress. Perubahan template master tidak mengubah snapshot SPK yang sudah dibuat.</div></div><div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}><span style={snapshotBadge}>{snapshotValid ? 'VALID · 100%' : `PERIKSA · ${(snapshotTotal * 100).toFixed(2)}%`}</span><span style={snapshotMeta}>{currentRows.length} kategori · sumber: SPK Progress Config</span></div></div><div className="kavio-table-wrap"><table className="kavio-table spk-detail-table"><thead><tr><th style={th}>#</th><th style={th}>Kategori</th><th style={th}>Bobot Final</th><th style={th}>Progress Akumulasi</th><th style={th}>Kontribusi</th></tr></thead><tbody>{currentRows.map((row) => <tr key={row.id_kategori}><td style={td}>{categoryMap.get(row.id_kategori)?.urutan ?? '—'}</td><td style={tdStrong}>{categoryMap.get(row.id_kategori)?.nama_kategori ?? row.id_kategori}</td><td style={tdStrong}>{(Number(row.bobot_final) * 100).toFixed(2)}%</td><td style={td}>{(Number(row.progress_akumulasi) * 100).toFixed(2)}%</td><td style={td}>{(Number(row.progress_berbobot) * 100).toFixed(2)}%</td></tr>)}{!currentRows.length && <tr><td colSpan={5} style={empty}>Konfigurasi bobot SPK belum tersedia.</td></tr>}</tbody></table></div></section>
        <section className="spk-detail-data-panel"><div className="spk-detail-section-title">Progress per Kategori</div><div className="kavio-table-wrap"><table className="kavio-table spk-detail-table"><thead><tr><th style={th}>Kategori</th><th style={th}>Bobot</th><th style={th}>Akumulasi</th><th style={th}>Berbobot</th><th style={th}>Update Terakhir</th></tr></thead><tbody>{currentRows.map((row) => <tr key={row.id_kategori}><td style={tdStrong}>{categoryMap.get(row.id_kategori)?.nama_kategori ?? row.id_kategori}</td><td style={td}>{(Number(row.bobot_final) * 100).toFixed(2)}%</td><td style={td}>{(Number(row.progress_akumulasi) * 100).toFixed(2)}%</td><td style={td}>{(Number(row.progress_berbobot) * 100).toFixed(2)}%</td><td style={td}>{row.tanggal_update_terakhir ? formatKavioDate(row.tanggal_update_terakhir) : 'Belum ada'}</td></tr>)}{!currentRows.length && <tr><td colSpan={5} style={empty}>Belum ada progress kategori.</td></tr>}</tbody></table></div></section>
