  ...
  return (
    <main style={{ minHeight: '100vh', background: '#f6f8fc', color: '#0f172a' }}>
      ...
      <section style={{ padding: 28, maxWidth: 1380, margin: '0 auto' }}>
        ...
        <section style={{ ...card, marginTop: 16, overflow: 'hidden' }}><div style={sectionTitle}>Informasi SPK</div><div style={{ padding: 18, display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0, 1fr))', gap: 16 }}><Info label="Tanggal SPK" value={spk.tgl_spk} /><Info label="Jenis Bobot" value={spk.jenis_bobot} /><Info label="Kantor/Pelaksana" value={office?.nama_kantor_pelaksana ?? spk.id_kantor} /><Info label="Mandor" value={mandor?.nama_mandor ?? spk.id_mandor} /><Info label="Update Terakhir" value={decision?.tanggal_update_terakhir ?? latestPeriod?.date ?? 'Belum ada'} /><Info label="Hari Sejak Update" value={decision?.tanggal_update_terakhir ? `${decision.hari_sejak_update} hari` : 'Belum ada'} /><Info label="Status SPK" value={spk.status_spk} /><Info label="Target Selesai" value={spk.tgl_target_selesai} /></div></section>
        <section style={{ ...card, marginTop: 16, padding: 18 }}><div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'center', flexWrap: 'wrap' }}><div><div style={sectionTitleNoPad}>Snapshot Bobot Final SPK</div><div style={mutedText}>Bobot ini adalah bobot final yang dipakai SPK untuk seluruh perhitungan progress. Perubahan template master tidak mengubah snapshot SPK yang sudah dibuat.</div></div><div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}><span style={snapshotBadge}>{snapshotValid ? 'VALID · 100%' : `PERIKSA · ${(snapshotTotal * 100).toFixed(2)}%`}</span><span style={snapshotMeta}>{currentRows.length} kategori · sumber: SPK Progress Config</span></div></div><div style={{ marginTop: 14, overflowX: 'auto' }}><table style={table}><thead><tr><th style={th}>#</th><th style={th}>Kategori</th><th style={th}>Bobot Final</th><th style={th}>Progress Akumulasi</th><th style={th}>Kontribusi</th></tr></thead><tbody>{currentRows.map((row) => <tr key={row.id_kategori}><td style={td}>{categoryMap.get(row.id_kategori)?.urutan ?? '—'}</td><td style={tdStrong}>{categoryMap.get(row.id_kategori)?.nama_kategori ?? row.id_kategori}</td><td style={tdStrong}>{(Number(row.bobot_final) * 100).toFixed(2)}%</td><td style={td}>{(Number(row.progress_akumulasi) * 100).toFixed(2)}%</td><td style={td}>{(Number(row.progress_berbobot) * 100).toFixed(2)}%</td></tr>)}{!currentRows.length && <tr><td colSpan={5} style={empty}>Konfigurasi bobot SPK belum tersedia.</td></tr>}</tbody></table></div></section>
