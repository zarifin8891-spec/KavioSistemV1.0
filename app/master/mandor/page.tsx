...
export default async function MasterMandorPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const [{ data: mandor, error: mandorError }, { data: kantor, error: kantorError }] = await Promise.all([
    supabase
      .from('master_mandor')
      .select('id_mandor, nama_mandor, id_kantor, no_hp, status_aktif, keterangan')
      .order('nama_mandor'),
    supabase
  ...
  const rows = (mandor ?? []) as Mandor[];
  const kantorRows = (kantor ?? []) as Kantor[];
  const kantorMap = new Map(kantorRows.map((item) => [item.id_kantor, item.nama_kantor_pelaksana]));
  const pageError = params.error ?? mandorError?.message ?? kantorError?.message;
    ...
    <main style={{ minHeight: '100vh', background: '#f6f8fc', color: '#0f172a' }}>
      <header style={header}>
        <div>
          ...
          <div style={title}>Master Mandor</div>
        </div>
      ...
      <section style={{ padding: 28, maxWidth: 1180, margin: '0 auto' }}>
        <div style={{ marginBottom: 22 }}>
          <h1 style={{ margin: '0 0 6px', fontSize: 28 }}>Data Mandor</h1>
          <p style={{ margin: 0, color: '#64748b' }}>Master mandor yang terhubung ke kantor/pelaksana untuk menjadi pilihan pada SPK.</p>
        </div>

        {pageError && <div style={alertError}>{pageError}</div>}
        {params.success && <div style={alertSuccess}>{params.success}</div>}
        {!kantorRows.length && !kantorError && (
          <div style={alertWarning}>Belum ada kantor pelaksana aktif. Tambahkan kantor terlebih dahulu sebelum membuat mandor.</div>
        )}

        <section style={card}>
          <div style={sectionTitle}>Tambah Mandor</div>
          <form action={createMandor} style={{ display: 'grid', gridTemplateColumns: '1fr 2fr 2fr 1.5fr', gap: 12, padding: 18 }}>
            <Field name="id_mandor" label="ID Mandor" placeholder="MDR05" />
            <Field name="nama_mandor" label="Nama Mandor" placeholder="Nama lengkap mandor" />
            ...
            <div style={{ gridColumn: '1 / -1', textAlign: 'right' }}>
              <button type="submit" style={primaryButton} disabled={!kantorRows.length}>+ Simpan Mandor</button>
            </div>
          </form>
        </section>

        <section style={{ ...card, marginTop: 20, overflow: 'hidden' }}>
          <div style={sectionTitle}>Daftar Mandor ({rows.length})</div>
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
              <thead>
                <tr style={{ background: '#f8fafc', textAlign: 'left' }}>
                  <th style={th}>ID Mandor</th>
                  <th style={th}>Nama Mandor</th>
                  <th style={th}>Kantor / Pelaksana</th>
              ...
              <tbody>
                {rows.map((row) => (
                  <tr key={row.id_mandor}>
                    <td style={tdStrong}>{row.id_mandor}</td>
                    <td style={td}>{row.nama_mandor}</td>
                    ...
                    <td style={td}>
                      <form action={toggleMandor}>
                        <input type="hidden" name="id_mandor" value={row.id_mandor} />
...
const alertWarning = { background: '#fffbeb', border: '1px solid #fde68a', color: '#92400e', padding: 12, borderRadius: 10, marginBottom: 14 };
