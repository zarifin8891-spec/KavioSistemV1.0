...
export default async function MasterKategoriPekerjaanPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const { data, error } = await supabase
    ...
    .select('id_kategori, nama_kategori, urutan, status_aktif')
    ...
    <main style={{ minHeight: '100vh', background: '#f6f8fc', color: '#0f172a' }}>
      <header style={header}>
        <div>
          ...
          <div style={title}>Master Kategori Pekerjaan</div>
        </div>
      ...
      <section style={{ padding: 28, maxWidth: 1180, margin: '0 auto' }}>
        <div style={{ marginBottom: 22 }}>
          <h1 style={{ margin: '0 0 6px', fontSize: 28 }}>Data Kategori Pekerjaan</h1>
          <p style={{ margin: 0, color: '#64748b' }}>Master kategori pekerjaan yang menjadi dasar bobot progress SPK.</p>
        </div>
        ...
        <section style={card}>
          <div style={sectionTitle}>Tambah Kategori Pekerjaan</div>
          <form action={createKategoriPekerjaan} style={{ display: 'grid', gridTemplateColumns: '1fr 2fr 1fr', gap: 12, padding: 18 }}>
            <Field name="id_kategori" label="ID Kategori" placeholder="KAT01" required />
            <Field name="nama_kategori" label="Nama Kategori" placeholder="Pekerjaan Pondasi" required />
            <NumberField name="urutan" label="Urutan" placeholder="1" required />
            <div style={{ gridColumn: '1 / -1', textAlign: 'right' }}>
              <button type="submit" style={primaryButton}>+ Simpan Kategori</button>
            </div>
          </form>
        </section>

        <section style={{ ...card, marginTop: 20, overflow: 'hidden' }}>
          <div style={sectionTitle}>Daftar Kategori ({rows.length})</div>
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
              <thead><tr style={{ background: '#f8fafc', textAlign: 'left' }}>
                <th style={th}>Urutan</th><th style={th}>ID</th><th style={th}>Nama Kategori</th><th style={th}>Aktif</th><th style={th}>Aksi</th>
              </tr></thead>
              <tbody>
                {rows.map((row) => (
                  <tr key={row.id_kategori}>
                    <td style={td}>{row.urutan}</td>
                    <td style={tdStrong}>{row.id_kategori}</td>
                    <td style={td}>{row.nama_kategori}</td>
                    <td style={td}><span style={row.status_aktif ? activeBadge : inactiveBadge}>{row.status_aktif ? 'AKTIF' : 'NONAKTIF'}</span></td>
                    <td style={td}>
                      <form action={toggleKategoriPekerjaan}>
                ...
                {!rows.length && <tr><td colSpan={5} style={{ ...td, textAlign: 'center', padding: 34, color: '#64748b' }}>Belum ada data kategori pekerjaan.</td></tr>}
              </tbody>
            </table>
          </div>
        </section>
...
function NumberField({ name, label, placeholder, required }: { name: string; label: string; placeholder: string; required?: boolean }) {
  return <label style={labelStyle}><span>{label}</span><input type="number" name={name} placeholder={placeholder} min="1" step="1" required={required} style={inputStyle} /></label>;
}
