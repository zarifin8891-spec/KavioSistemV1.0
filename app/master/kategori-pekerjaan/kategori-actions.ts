...
function integer(value: FormDataEntryValue | null) { const n = Number(String(value ?? '').trim()); return Number.isInteger(n) ? n : NaN; }
...
export async function createKategoriPekerjaan(formData: FormData) {
  ...
  if (!idKategori || !namaKategori) redirect('/master/kategori-pekerjaan?error=ID%20dan%20nama%20kategori%20wajib%20diisi');
  if (!Number.isInteger(urutan) || urutan < 1) redirect('/master/kategori-pekerjaan?error=Urutan%20harus%20minimal%201');
  const { error } = await supabase.from('master_kategori_pekerjaan').insert({ id_kategori: idKategori, nama_kategori: namaKategori, urutan, status_aktif: true });
...
export async function toggleKategoriPekerjaan(formData: FormData) {
  ...
  const { error } = await supabase.from('master_kategori_pekerjaan').update({ status_aktif: !statusAktif }).eq('id_kategori', idKategori);
  if (error) redirect(`/master/kategori-pekerjaan?error=${encodeURIComponent(error.message)}`);
  revalidatePath('/master/kategori-pekerjaan');
  revalidatePath('/dashboard');
