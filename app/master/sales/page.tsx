...
export default async function SalesPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const count = (status: string) => active.filter((r) => r.status_sales === status).length;
    ...
    <main className="sales-page" style={page}>
      <section style={content}>
        <div style={pageHeader}>
          <div>
            <h1 style={h1}>SALES MANAGEMENT</h1>
        ...
        <section style={card}>
          <div style={filterRow}>
            <select style={filterInput} defaultValue="">
            ...
            <select style={filterInput} defaultValue="">
              ...
              {types.map((type) => <option key={type.id_tipe} value={type.id_tipe}>{type.nama_tipe.toUpperCase()}</option>)}
            </select>
          ...
          <div style={{ overflowX: 'auto' }}>
            <table style={table}>
              ...
              <tbody>
                {sales.map((row, index) => {
                  const kavling = kavlings.find((item) => item.id_kavling === row.id_kavling);
                  return (
                    <tr key={row.id_sales}>
                      <td style={tdCenter}>{index + 1}</td>
                      <td style={td}>{row.tgl_booking ?? '—'}</td>
                      <td style={tdStrong}><Link href={`/master/sales/detail?id=${row.id_sales}`} style={salesLink}>{row.id_kavling}</Link></td>
                      <td style={tdStrong}>{row.nama_konsumen}</td>
                      <td style={td}>{typeMap.get(kavling?.id_tipe ?? '') ?? '—'}</td>
                      <td style={td}>{formatCurrency(row.harga_jual)}</td>
                      <td style={td}>{row.jenis_pembayaran ?? '—'}</td>
                      <td style={td}>{row.jenis_pembayaran === 'KPR' ? (bankMap.get(row.id_bank ?? '') ?? '—') : '—'}</td>
                      <td style={td}><span style={salesBadge(row.status_sales)}>{statusLabel(row.status_sales)}</span></td>
                      <td style={td}>
                        {row.status_aktif ? (
                          <div style={actionsCell}>
                            <Link href={`/master/sales/detail?id=${row.id_sales}`} style={detailButton}>DETAIL</Link>
                            <form action={deactivateSales}>
                              <input type="hidden" name="id_sales" value={row.id_sales} />
                              <button type="submit" style={detailButton}>TUTUP</button>
                            </form>
                          </div>
                        ) : <span style={{ color: '#8D815F' }}>—</span>}
                      </td>
                    </tr>
                  );
                })}
                {!sales.length && <tr><td colSpan={10} style={empty}>BELUM ADA DATA SALES.</td></tr>}
              </tbody>
            </table>
          </div>
...
function Summary({ label, value }: { label: string; value: number }) {
  return <div style={summaryCard}><div style={summaryLabel}>{label}</div><div style={summaryValue}>{value}</div></div>;
}
...
function salesBadge(status: string) {
  const tone = status === 'BATAL' ? ['rgba(248,113,113,.14)', '#FCA5A5', '#B91C1C'] : status === 'AKAD' ? ['rgba(134,239,172,.14)', '#BBF7D0', '#15803D'] : status === 'DP' ? ['rgba(232,204,122,.20)', '#172B43', '#D8B45A'] : ['rgba(92,171,239,.18)', '#DCEEFF', '#4D91C8'];
  return { display: 'inline-flex', padding: '6px 11px', borderRadius: 7, fontSize: 11, fontWeight: 900, background: tone[0], color: tone[1], border: `1px solid ${tone[2]}`, whiteSpace: 'nowrap' as const };
}

const page = { minHeight: '100%', background: 'transparent', color: '#F7F3E8' };
const content = { maxWidth: 1500, margin: '0 auto', padding: '0 0 30px' };
const pageHeader = { display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 20, marginBottom: 18 };
const h1 = { margin: 0, color: '#F7F3E8', fontSize: 32, fontWeight: 900, letterSpacing: 1.2 };
const subtitle = { margin: '2px 0 0', color: '#C9BC99', fontSize: 12, fontWeight: 700, letterSpacing: .2 };
...
const card = { background: 'linear-gradient(180deg,#123150,#081F39)', border: '1px solid rgba(216,180,90,.34)', borderRadius: 14, boxShadow: '0 10px 28px rgba(0,0,0,.18)', overflow: 'hidden' as const };
