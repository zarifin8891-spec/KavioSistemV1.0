...
export default async function DashboardPage() {
  ...
  const { data, error } = await supabase
    ...
    .select('id_spk, id_kavling, progress_aktual, progress_seharusnya, gap_progress, sisa_hari, tanggal_update_terakhir, progress_periode_terakhir, status_operasional, status_ritme, prioritas_tindakan, action_rekomendasi, tgl_target_selesai')
  ...
  const actionRows = rows.filter((row) => row.prioritas_tindakan !== 'NORMAL').sort((a, b) => priorityRank(a.prioritas_tindakan) - priorityRank(b.prioritas_tindakan));
  ...
  return (
    <main style={{ minHeight: '100vh', background: '#f6f8fc', color: '#0f172a' }}>
      ...
      <section style={{ padding: 28, maxWidth: 1480, margin: '0 auto' }}>
        ...
        <section style={{ ...card, marginBottom: 16, border: actionRows.length ? '1px solid #fed7aa' : '1px solid #e2e8f0' }}>
          <div style={{ padding: 18, borderBottom: '1px solid #e2e8f0' }}><div style={{ fontWeight: 800, fontSize: 16 }}>Action Center</div><div style={{ color: '#64748b', fontSize: 13, marginTop: 4 }}>Jawaban Decision Engine atas pertanyaan: <strong>SPK mana yang membutuhkan tindakan sekarang?</strong></div></div>
          {actionRows.length ? <div style={{ overflowX: 'auto' }}><table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14, minWidth: 980 }}><thead><tr style={{ background: '#fffaf0', textAlign: 'left' }}><th style={th}>Prioritas</th><th style={th}>Kavling</th><th style={th}>Status</th><th style={th}>Ritme</th><th style={th}>Sisa Hari</th><th style={th}>Update Terakhir</th><th style={th}>Tindakan Direkomendasikan</th></tr></thead><tbody>{actionRows.slice(0, 12).map((row) => <tr key={row.id_spk}><td style={td}><PriorityBadge priority={row.prioritas_tindakan} /></td><td style={td}><Link href={`/master/spk/detail/${row.id_spk}`} style={kavlingLink}>{row.id_kavling}</Link><div style={{ fontSize: 11, color: '#94a3b8', marginTop: 3 }}>SPK {row.id_spk.slice(0, 8)}</div></td><td style={td}><StatusBadge status={row.status_operasional} /></td><td style={td}><PaceBadge status={row.status_ritme} /></td><td style={{ ...td, fontWeight: 800, color: row.sisa_hari < 0 ? '#b91c1c' : row.sisa_hari <= 7 ? '#b45309' : '#0f172a' }}>{row.sisa_hari < 0 ? `Lewat ${Math.abs(row.sisa_hari)} hari` : `${row.sisa_hari} hari`}</td><td style={td}>{row.tanggal_update_terakhir ?? 'Belum ada'}</td><td style={{ ...td, fontWeight: 700 }}>{row.action_rekomendasi}</td></tr>)}</tbody></table></div> : <div style={{ padding: 28, color: '#15803d', fontWeight: 700 }}>Semua SPK berada dalam kondisi normal. Belum ada tindakan prioritas.</div>}
        </section>
...
function priorityRank(priority: ActionPriority) { return priority === 'TINGGI' ? 0 : priority === 'SEDANG' ? 1 : 2; }
...
function PriorityBadge({ priority }: { priority: ActionPriority }) { const s = priorityStyles[priority]; return <span style={{ display: 'inline-flex', alignItems: 'center', padding: '5px 9px', borderRadius: 999, fontSize: 10, fontWeight: 800, letterSpacing: 0.3, background: s.background, color: s.color, border: `1px solid ${s.border}` }}>{priority}</span>; }
...
const priorityStyles: Record<ActionPriority, { background: string; color: string; border: string }> = { TINGGI: { background: '#fef2f2', color: '#b91c1c', border: '#fecaca' }, SEDANG: { background: '#fffbeb', color: '#b45309', border: '#fde68a' }, NORMAL: { background: '#f8fafc', color: '#475569', border: '#cbd5e1' } };
...
const sectionTitle = { padding: 18, fontWeight: 800, fontSize: 15 };
